...
class CrossAttention2d(nn.Module):
    ...
    """
    def __init__(self, enc_channels, dec_channels, num_heads=8):
        super().__init__()
        # project decoder features to query dimension matching encoder
        self.q_proj = nn.Conv2d(dec_channels, enc_channels, 1)
        self.k_proj = nn.Conv2d(enc_channels, enc_channels, 1)
        self.v_proj = nn.Conv2d(enc_channels, enc_channels, 1)
        self.mha = nn.MultiheadAttention(embed_dim=enc_channels, num_heads=num_heads, batch_first=True)
        self.norm = nn.LayerNorm(enc_channels)

    def forward(self, dec_feat, enc_feat):
        # dec_feat: (B, D, H, W), enc_feat: (B, C, H, W)
        B, C, H, W = enc_feat.shape
        # project
        Q = self.q_proj(dec_feat).view(B, C, H * W).permute(0, 2, 1)  # (B, N, C)
        K = self.k_proj(enc_feat).view(B, C, H * W).permute(0, 2, 1)
        V = self.v_proj(enc_feat).view(B, C, H * W).permute(0, 2, 1)
        attn_out, _ = self.mha(Q, K, V)
        out = self.norm(attn_out + Q)
        out = out.permute(0, 2, 1).view(B, C, H, W)
        # gate encoder features
        gate = torch.sigmoid(out)
        return gate * enc_feat
...
class MultiHeadUNet(nn.Module):
    def __init__(self, in_ch=1, base_ch=64, dropout=0.3):
        ...
        self.mhsa = MHSA2d(base_ch*16)

        # cross-attention modules for skip connections
        self.ca4 = CrossAttention2d(enc_channels=base_ch*8, dec_channels=base_ch*8)
        self.ca3 = CrossAttention2d(enc_channels=base_ch*4, dec_channels=base_ch*4)
        self.ca2 = CrossAttention2d(enc_channels=base_ch*2, dec_channels=base_ch*2)
        self.ca1 = CrossAttention2d(enc_channels=base_ch,   dec_channels=base_ch)
    ...
    def forward(self, x):
        ...
        a = self.up4_a(b)
        f4 = self.ca4(a, d4)
        a = torch.cat([a, f4], dim=1)
        a = self.dec4_a(a)

        a = self.up3_a(a)
        f3 = self.ca3(a, d3)
        a = torch.cat([a, f3], dim=1)
        a = self.dec3_a(a)

        a = self.up2_a(a)
        f2 = self.ca2(a, d2)
        a = torch.cat([a, f2], dim=1)
        a = self.dec2_a(a)

        a = self.up1_a(a)
        f1 = self.ca1(a, d1)
        a = torch.cat([a, f1], dim=1)
        a = self.dec1_a(a)
        out_a = self.out_a(a)

        # head B (small vessels)
        b2 = self.up4_b(b)
        f4b = self.ca4(b2, d4)
        b2 = torch.cat([b2, f4b], dim=1)
        b2 = self.dec4_b(b2)

        b2 = self.up3_b(b2)
        f3b = self.ca3(b2, d3)
        b2 = torch.cat([b2, f3b], dim=1)
        b2 = self.dec3_b(b2)

        b2 = self.up2_b(b2)
        f2b = self.ca2(b2, d2)
        b2 = torch.cat([b2, f2b], dim=1)
        b2 = self.dec2_b(b2)

        b2 = self.up1_b(b2)
        f1b = self.ca1(b2, d1)
        b2 = torch.cat([b2, f1b], dim=1)
        b2 = self.dec1_b(b2)
        out_b = self.out_b(b2)
...
save_dir = "/public3/Xinyu/3D_tissue/IF/Vascular_IF/MultiClassification_model/multiHead/HML_model/Attention"
...
for epoch in range(1, epochs + 1):
    ...
    if (acc1+acc2) > best_acc:
        best_acc = acc1+acc2
        no_imp   = 0
        torch.save(net.state_dict(), os.path.join(save_dir, "best_multihead.attention.pth"))
    else:
        no_imp += 1
        if no_imp >= patience:
