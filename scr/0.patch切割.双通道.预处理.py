...
# 输出目录
base_out    = f'/public3/Xinyu/3D_tissue/IF/Vascular_IF/MultiClassification_model/multiHead/HML_model/data_enh'
...
# —— 读取组织轮廓 ——
contour = cv2.imread(f'/public3/Xinyu/3D_tissue/IF/Vascular_IF/MultiClassification_model/multiHead/HML_model/{sample}_tissue_contours.png', cv2.IMREAD_GRAYSCALE)
...
canvas[..., 2] = cd31_enh                          # 第三通道：CD31 原始灰度
cv2.imwrite(os.path.join('/public3/Xinyu/3D_tissue/IF/Vascular_IF/MultiClassification_model/multiHead/HML_model', "overlay.png"), canvas)
...
for i in range(n_rows):
    for j in range(n_cols):
        ...
        c_p = cd31_enh[y0:y1, x0:x1]
        d_p = dapi_enh[y0:y1, x0:x1]
        m1_p = m1[y0:y1, x0:x1]
        m2_p = m2[y0:y1, x0:x1]
