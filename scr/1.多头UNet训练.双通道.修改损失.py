...
save_dir = "/public3/Xinyu/3D_tissue/IF/Vascular_IF/MultiClassification_model/multiHead/twoChannel_CGL_v2"
...
def dice_loss(input_probs, target, eps=1e-6):
    # input_probs, target ∈ [0,1]
    num = 2*(input_probs * target).sum(dim=[1,2,3]) + eps
    den =   input_probs.sum(dim=[1,2,3]) + target.sum(dim=[1,2,3]) + eps
    return 1 - (num/den).mean()

def multihead_loss(logits1, logits2, mask1, mask2,
                   alpha=1.0, beta=1.0, gamma=1.0):
    """
    logits1, logits2: raw outputs (B,1,H,W)
    mask1, mask2: binary targets {0,1} same shape
    alpha: excl 权重; beta: union-dice 权重; gamma: optional focal权重
    """
    # 1. 基础 BCE
    bce1 = F.binary_cross_entropy_with_logits(logits1, mask1)
    bce2 = F.binary_cross_entropy_with_logits(logits2, mask2)

    # 2. 互斥惩罚
    p1 = torch.sigmoid(logits1)
    p2 = torch.sigmoid(logits2)
    loss_excl = (p1 * p2).mean()

    # 3. 并集 Dice
    union_p = p1 + p2 - p1*p2
    union_t = ((mask1+mask2)>0).float()
    loss_diceU = dice_loss(union_p, union_t)

    return bce1 + bce2 + alpha * loss_excl + beta  * loss_diceU
...
for epoch in range(1, epochs + 1):
    ...
    for img, m1, m2 in tqdm(train_loader, desc=f"Epoch {epoch}/{epochs} Train"):
        ...
        with autocast():
            out1, out2 = net(img)
            loss = multihead_loss(out1, out2, m1, m2, alpha = 0.5, beta = 1.0)
    ...
    train_total.append(L_total / len(train_loader))
    ...
    print(f"Epoch {epoch} → train_loss_large: {train_total[-1]:.4f}, "
...
plt.subplot(1, 2, 1)
plt.plot(epochs, train_total, 'o-', label = "Loss total")
