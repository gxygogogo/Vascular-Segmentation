...
output_dir   = f'/public3/Xinyu/3D_tissue/IF/Vascular_IF/MultiClassification_model/multiHead/twoChannel/{sample_name}_enh'
patch_size   = 256
overlap_frac = 0.75
...
cd31 = cv2.normalize(raw[2].astype(np.float32), None, 0, 1, cv2.NORM_MINMAX)

del img, raw
gc.collect()

# —— 读取组织轮廓 ——
contour = cv2.imread(f'/public3/Xinyu/3D_tissue/IF/Vascular_IF/MultiClassification_model/multiHead/twoChannel/{sample_name}_enh/{sample_name}_tissue_contours.png', cv2.IMREAD_GRAYSCALE)
...
# 1) 同样先转到 uint8
dapi_u8 = (dapi_mask * 255).clip(0,255).astype(np.uint8)
cd31_u8 = (cd31_mask * 255).clip(0,255).astype(np.uint8)

del cd31_mask, dapi_mask
gc.collect()

# 2) 生成 LUT 表
gamma = 1.2  # >1 提升亮部，对比度增强；<1 则增强暗部
invGamma = 1.0 / gamma
lut = np.array([((i/255.0)**invGamma)*255 for i in np.arange(256)]).astype("uint8")

dapi_enh = cv2.LUT(dapi_u8, lut)
cd31_enh = cv2.LUT(cd31_u8, lut)
...
dapi_pad = np.pad(dapi_enh.astype(np.float32) / 255, ((0,pad_h),(0,pad_w)), mode = "reflect")
cd31_pad = np.pad(cd31_enh.astype(np.float32) / 255, ((0,pad_h),(0,pad_w)), mode = "reflect")
H_p, W_p = dapi_pad.shape

del dapi_enh, cd31_enh
gc.collect()
